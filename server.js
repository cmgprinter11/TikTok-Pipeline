...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  input, textarea { width: 100%; background: #0d0d0d; border: 1.5px solid #1f1f1f; border-radius: 6px; color: #ccc; font-size: 12px; padding: 10px 12px; outline: none; font-family: inherit; }
  .url-row { display: flex; gap: 10px; }
  .url-row input { flex: 1; }
  .btn-primary { background: #ff6b2b; color: #fff; border: none; border-radius: 6px; padding: 0 20px; height: 40px; font-size: 12px; font-weight: 700; cursor: pointer; white-space: nowrap; letter-spacing: 0.05em; }
  ...
  .mode-btn { background: #111; border: 1.5px solid #1f1f1f; border-radius: 8px; padding: 11px 14px; cursor: pointer; text-align: left; color: inherit; }
  ...
  .avatar-desc { font-size: 10px; color: #3a3a3a; }
  ...
  .error { background: #1a0a0a; border: 1.5px solid #ff3333; border-radius: 8px; padding: 12px 16px; font-size: 12px; color: #ff6666; margin-bottom: 14px; display: none; }
...
const AVATARS = [
  ...
  { id: "monkey", name: "Monkey Onesie", desc: "5'4 girl inside full monkey costume suit. Full monkey face visible, mouth closed, no human hair showing, skinny tail hanging down." },
...
let selectedAvatar = "hailey";
let selectedProduct = "lamp";
...
function renderList(items, containerId, selectedId, type) {
  ...
  container.innerHTML = items.map(item => \`
    <div class="\${type}-item \${item.id === selectedId ? 'selected' : ''}" onclick="select\${type.charAt(0).toUpperCase()+type.slice(1)}('\${item.id}')">
      <div class="avatar-icon">\${item.name[0]}</div>
      <div>
        <div class="avatar-name">\${item.name}</div>
        <div class="avatar-desc">\${item.id !== 'custom' ? item.desc.slice(0,45)+'...' : 'Describe below'}</div>
      </div>
    </div>
  \`).join('');
}
...
async function analyze() {
  ...
  try {
    ...
    if (data.metadata) {
      ...
      const stats = [
        m.views ? \`<div class="stat"><div class="stat-val">👁 \${Number(m.views).toLocaleString()}</div><div class="stat-label">views</div></div>\` : '',
        m.likes ? \`<div class="stat"><div class="stat-val">❤️ \${Number(m.likes).toLocaleString()}</div><div class="stat-label">likes</div></div>\` : '',
        m.shares ? \`<div class="stat"><div class="stat-val">↗️ \${Number(m.shares).toLocaleString()}</div><div class="stat-label">shares</div></div>\` : '',
        m.author ? \`<div class="stat"><div class="stat-val" style="font-size:11px">@\${m.author}</div><div class="stat-label">creator</div></div>\` : '',
      ].join('');
...
app.post("/analyze", async (req, res) => {
  ...
  try {
    ...
    res.json({
      result: prompt,
      metadata: { views: metadata.playCount, likes: metadata.diggCount, shares: metadata.shareCount, caption: metadata.text, author: metadata.authorMeta?.name },
    });
  } catch (err) {
    console.error(err);
